...
import com.gram.gram_landlord.sdk.game.protocols.request.CancelReadyRequest;
import com.gram.gram_landlord.sdk.game.protocols.request.CardsOutRequest;
import com.gram.gram_landlord.sdk.game.protocols.request.ChatMsgRequest;
import com.gram.gram_landlord.sdk.game.protocols.request.EndGameRequest;
import com.gram.gram_landlord.sdk.game.protocols.request.EndGrabLandlordRequest;
import com.gram.gram_landlord.sdk.game.protocols.request.ExitSeatRequest;
import com.gram.gram_landlord.sdk.game.protocols.request.GiveUpLandlordRequest;
import com.gram.gram_landlord.sdk.game.protocols.request.LandlordMultipleWagerRequest;
import com.gram.gram_landlord.sdk.game.protocols.request.MultipleWagerRequest;
import com.gram.gram_landlord.sdk.game.protocols.request.ReadyRequest;
...
public class GameActivity extends BaseActivity {
                ...
                RxView.clicks(ready)
                        ...
                        .subscribe(o -> {
                            if (!isReady) {
                                ...
                                GameClient.getClient().send(readyRequest);
                            } else {
                                CancelReadyRequest cancelReadyRequest = new CancelReadyRequest(true);
                                GameClient.getClient().send(cancelReadyRequest);
                            }
    ...
    public void onAssistResponse(Response response) {
        if(response.getKey().equals(RequestKey.GET_PLAYER_INFO)) {
            EventBus.getDefault().removeStickyEvent(response);
            if(CommonUtil.checkNotNull(left) && response.get("username").equals(left) && response.getPiture() != null) {
                rivalLeftAvatar.setImageBitmap(AvatarChangeUtil.bytes2Bitmap(response.getPiture()));
            }
            if(CommonUtil.checkNotNull(right) && response.get("username").equals(right) && response.getPiture() != null) {
                rivalRightAvatar.setImageBitmap(AvatarChangeUtil.bytes2Bitmap(response.getPiture()));
            }
        }
    }
...
//                                }
     ...
     * @param name 对手用户名
     */
    private void requestRivalsInfo(String name) {
